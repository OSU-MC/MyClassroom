...
function LoggedIn(props){
    ...
    const navItems = [
        {path: '/', label: 'Courses'},
        {path: '/login', label: 'Logout', className: 'right', onClick: handleLogout},
    ...
    return(
        <>
           {navItems.map((item) => (
                <a href={item.path} className={`page ${item.className} ${pathname === item.path ? 'curr' : ''}`} onClick={item.onClick}>
                    {item.label}
               </a>
           ))}
        </>
